...
### I want to make bigger edits to text

If you can, draft the changes you want to write in a Google doc or some other text editor first. 

Go to [the CMS](https://kiara-website.pages.dev/keystatic) (if you have access) and find the appopriate collection for your page. This loosely matches up with the main sidebar sections, and with our target user personas. Find the page you want to work on by its "slug" - probably the same as the title but with `-` not spaces. Paste in or make your edits in the big text box for that page, then hit the blue "Save" button in the top corner. 
...
Also use this process if you want to add a new page, when you've already drafted the content elsewhere and it's mostly text.

### I want to add a new, complicated page 

For these purposes, a complicated page is one with a lot of code samples, requiring special components or extra markdown things that the CMS doesn't support well. I'm assuming that you know the basics of how to use `git` to clone a repository, make a branch, commit and pull request, can run commands in your terminal, and can (work out how to) write markdown syntax in your text editor of choice.
...
You'll need to write your content using markdown syntax directly - [here's a quick primer on how to do that](https://www.markdownguide.org/cheat-sheet/). Starlight also provides a couple extra neat things that might be useful (if you're in a `.md` or `.mdx` file, not a `.mdoc` one yet). One is  [asides](https://starlight.astro.build/guides/authoring-content/#asides) for things you want to draw extra attention to:

:::tip
Here's an example of an aside. They might be useful for warning people about particular tricky things or common mistakes
:::

The other is [extra fancy things with code blocks](https://starlight.astro.build/guides/authoring-content/#expressive-code-features) using the [Expressive Code](https://expressive-code.com/) library. This can be really powerful for highligting particular lines or diffs in your code samples, making it clear whether you're showing code to write in a file or run in a terminal. Read their docs or ask for help on how to do this. 

When you're done making changes, check that the site will (probably) build with 
...
This will probably give you a lot of output that you don't care about (because there's a lot of pages!), just check that the final like of output says `Complete!` rather than an error message or stack trace. See [when things go wrong](#when-things-go-wrong) for ideas about how to fix error if there are any.


Once you're happy, make a git commit (probably to a new branch), push it and open a pull request to get review on what you've written. 
...
I'll use `kiara_plugin.core_types` as an example here, replace with the name of your plugin as appropriate. This process is a bit manual at the moment, but hopefully shouldn't be super common. If this changes, or I have spare time, I'll automate it. 
...
You only need to care about this if you particularly want to make a lot of changes to a lot of docs all at once (and only then to save you a bit of time), or preview how complicated pages are going to look. Don't worry about this section at all if that's not something you need to do. 
...
If this doesn't fix things, ping Caro (`@caro401` on GitHub). For the moment (because of how and where the site is currently deployed), you can't get any better diagonstics than "something failed", but this should be improved in the future. 
